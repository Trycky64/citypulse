...
export async function getAirQuality(lat, lon) {
    ...
    return cacheSWR(key, async () => {
        try {
            ...
            const pm25Arr = Array.isArray(hourly.pm2_5)
                ? hourly.pm2_5
                : [];
            const pm10Arr = Array.isArray(hourly.pm10)
                ? hourly.pm10
                : [];
            const no2Arr = Array.isArray(hourly.nitrogen_dioxide)
                ? hourly.nitrogen_dioxide
                : [];
            const o3Arr = Array.isArray(hourly.ozone)
                ? hourly.ozone
                : [];
            ...
            const aqiApprox = lastPM25 == null || Number.isNaN(lastPM25)
                ? 0
